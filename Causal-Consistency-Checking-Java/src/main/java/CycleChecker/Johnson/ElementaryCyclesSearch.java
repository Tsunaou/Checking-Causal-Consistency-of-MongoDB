...
//				System.out.print(" "+this.adjList[i][j]);
//			}
//			System.out.println();
		}
	}
	...
	public List getElementaryCycles() {
		...
		StrongConnectedComponents sccs = new StrongConnectedComponents(this.adjList); //得到强连通分量
		int s = 0;
	...
	private boolean findCycles(int v, int s, Vector[] adjList) {
		boolean f = false; // 表示是否找到一个环路
		this.stack.add(new Integer(v));
//		System.out.println(this.stack);
		this.blocked[v] = true;

		for (int i = 0; i < adjList[v].size(); i++) {
			int w = ((Integer) adjList[v].get(i)).intValue(); // w: next neighbor of v
			// found cycle
			if (w == s) {
//				System.out.println("Find Cycle "+ w);
		...
		if (f) {
			this.unblock(v);
//			System.out.println("Unblock "+ v);
		} else {
//			System.out.println("Start Blocking by "+ v + ".adj");
			for (int i = 0; i < adjList[v].size(); i++) {
				int w = ((Integer) adjList[v].get(i)).intValue();
				if (!this.B[w].contains(new Integer(v))) {
//					System.out.println("Block " + w + " Add "+ v);
